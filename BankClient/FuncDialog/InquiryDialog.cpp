#include "InquiryDialog.h"
#include "ui_InquiryDialog.h"
#include "mark.h"
#include <QMessageBox>
#include <QByteArray>

InquiryDialog::InquiryDialog(QWidget *parent,int client_sock) :
    QDialog(parent),
    ui(new Ui::InquiryDialog),m_iClientSfd(client_sock)
{
    ui->setupUi(this);
    this->setWindowTitle("InquiryDialog");

    connect(ui->confirm,SIGNAL(clicked(bool)),this,SLOT(Confirm()));
    connect(ui->cancel,SIGNAL(clicked(bool)),this,SLOT(Cancel()));
}

InquiryDialog::~InquiryDialog()
{
    delete ui;
}

void InquiryDialog::Confirm()
{
    if(Warning())
    {
        return ;
    }

    QByteArray id = ui->id->text().toLatin1();
    QByteArray passwd = ui->password->text().toLatin1();

    m_pInquiry = Inquiry::GetInquiry();
    this->Result(m_pInquiry->Function(m_iClientSfd,id.data(),passwd.data()));
}

void InquiryDialog::Cancel()
{
    int ret = QMessageBox::question(this,"Exit?","Are you sure you want to quit?",QMessageBox::No|QMessageBox::Yes,QMessageBox::Yes);
    if(ret == QMessageBox::Yes)
    {
        reject();
    }
}

void InquiryDialog::Result(int action)
{
    switch(action)
    {
        case NO_ACCOUNT: //无帐号
        {
            QMessageBox::critical(this,"error","No Account !!");
        }break;

        case PASSWD_ERROR: //密码错误
        {
            QMessageBox::critical(this,"error","PassWord ERROR !!");
        }break;

        case INQU_SUCCESS: //查询成功
        {
            accept();
            char t[30];
            sprintf(t,"Account Balance : %s",m_pInquiry->GetMoney());
            QMessageBox::about(this,"success",t);
        }break;
    }
}

int InquiryDialog::Warning()
{
    if(ui->id->text().length() == 0)  //无ID
    {
        QMessageBox::warning(this,"warning","Please input ID !");
        return 1;
    }

    if(ui->password->text().length() < 6 )   //密码过短
    {
        QMessageBox::warning(this,"warning","passwd is too short");
        return 1;
    }

    if(ui->password->text().length() > 8 )   //密码过长
    {
        QMessageBox::warning(this,"warning","passwd is too long");
        return 1;
    }

    return 0;
}
